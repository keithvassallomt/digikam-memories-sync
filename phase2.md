...
- **Preview cost per trigger.** ~~A full preview on this library takes minutes. Person-scoped automatic runs, driven by which person's faces changed, would be a phase 3 optimisation once the fingerprints record per-person deltas.~~ Built. Both fingerprints now carry a hash per person, and a trigger that names exactly one of them starts a person-scoped run. Measured on the development library: reading the whole fingerprint plus all 133 per-person hashes over 13,756 face regions takes about 20 ms, the same as reading the whole-library value alone did, because it is the same scan. This supersedes the section 0 assumption that automatic runs always use the **All faces** scope; they use it whenever the change cannot be pinned on one person, and the fallback sweep always does.
