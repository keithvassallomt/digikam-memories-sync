...
from .constants import COMPANION_APP_NAME, DAV_NS, IMAGE_EXT
        ...
        *,
        ...
        if not self.recognize_installed:
            raise RecognizeNotInstalledError(
                ...
                "installed and enabled. DigiMem requires Recognize."
            )
...
<d:propfind xmlns:d="DAV:"><d:prop><d:resourcetype/></d:prop></d:propfind>""",
    ...
    def _probe_face_sync_app(self) -> dict[str, bool]:
        ...
        try:
            ...
            LOG.debug("Could not probe the %s companion API: %s", COMPANION_APP_NAME, e)
            return dict(NO_CAPABILITIES)
    ...
    def read_changes(self) -> fingerprint_module.Fingerprint:
        ...
        if not self.supports_change_fingerprint:
            raise NextcloudConnectionError(
                f"The Nextcloud {COMPANION_APP_NAME} app must be updated "
    ...
    def list_named_people(self) -> list[str]:
        if not self.supports_export:
            raise NextcloudConnectionError(
                f"The Nextcloud {COMPANION_APP_NAME} app must be updated "
                "before Memories faces can be read."
            )
        status, _, raw = self._request(
            "GET",
...
""".encode()
    ...
    def assign_person(
        ...
        if not detection.dav_parent:
            raise RuntimeError(
                ...
                f"Nextcloud {COMPANION_APP_NAME} app, then retry this change."
            )
    ...
    def insert_detection(
        ...
        if confirmed and not self.supports_confirmed_insert:
            raise RuntimeError(
                f"Update the Nextcloud {COMPANION_APP_NAME} app before "
                "adding a reviewed face."
            )
