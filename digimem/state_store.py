...
class StateStore:
    ...
    def _backfill(self, added: set[tuple[str, str]]) -> None:
        ...
        if ("face_links", "synced_name") in added:
            ...
            # first run after upgrading does not re-ask about faces DigiMem
            # synced itself.
            self.conn.execute(
            ...
            if found:
                ...
                self.conn.execute(
                    f"""UPDATE run_progress SET phase = 'waiting',
                        error = 'DigiMem restarted. This will continue where it stopped.'
            ...
            if run_ids:
                ...
                self.conn.execute(
                    f"""UPDATE run_progress SET phase = 'apply_failed',
                        error = 'DigiMem stopped before Apply finished.'
                        WHERE run_id IN ({placeholders})""",
                    run_ids,
                )
                self.conn.commit()
            ...
            if run_ids:
                ...
                self.conn.execute(
                    f"""UPDATE run_progress SET phase = 'failed',
                        error = 'DigiMem stopped before the preview finished.'
                        WHERE run_id IN ({placeholders})""",
                    run_ids,
                )
                self.conn.commit()
