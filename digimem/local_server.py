...
class DigiMemHTTPServer(ThreadingHTTPServer):
    daemon_threads = True

    def __init__(self, address: tuple[str, int], app: AppService):
        super().__init__(address, DigiMemHandler)
        self.app = app
        self.api_token = secrets.token_urlsafe(24)


class DigiMemHandler(BaseHTTPRequestHandler):
    server: DigiMemHTTPServer
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/health":
            ...
            raw = raw.replace("__DIGIMEM_TOKEN__", self.server.api_token)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            self._json(
                HTTPStatus.INTERNAL_SERVER_ERROR,
                {"error": "The operation failed. Check the DigiMem terminal for details."},
            )
    ...
    def _authorized(self) -> bool:
        return self.headers.get("X-DigiMem-Token", "") == self.server.api_token
...
def create_server(config_dir: Path | None = None, port: int = 0) -> DigiMemHTTPServer:
    ...
    return DigiMemHTTPServer(("127.0.0.1", port), AppService(settings, state))
