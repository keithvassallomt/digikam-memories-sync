...
export function subline(status) {
  ...
  switch (status.state) {
    case 'setup':
      return 'DigiMem needs your digiKam folder and your Nextcloud account.';
    case 'syncing': {
      const run = status.run || {};
...
export function detail(status) {
  ...
  if (status.state === 'off') {
    return 'Turn it on and DigiMem keeps both libraries in step by itself.';
  }
  return '';
}
