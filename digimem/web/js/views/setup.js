...
export function create({ go, refresh }) {
  ...
  function showForm(settings) {
    ...
    replace(body,
      ...
      h('p', { class: 'lead' }, 'You only need to do this once. DigiMem saves these settings on this computer.'),
      card(
  ...
  function showFinish(settings) {
    ...
    replace(body,
      ...
      h('p', { class: 'lead' }, 'Last step. DigiMem can keep both libraries in step on its own.'),
      card(
        h('div', { class: 'lib-row' },
      ...
      card(
        h('label', { class: 'check' }, automatic,
          h('span', {},
            ...
            h('small', {},
              'DigiMem checks for changes in the background, waits until digiKam is closed before changing it, and only asks you when two names disagree.'))),
        h('label', { class: 'check' }, atLogin,
          h('span', {},
            h('strong', {}, 'Start DigiMem when I log in'),
            h('small', {}, 'Without this, syncing only happens while DigiMem is running.')))),
      message,
      h('div', { class: 'actions' },
        button('Finish', {
