// The screen that answers "what is DigiMem doing?" in one line.
...
export function create({ go, refresh }) {
  ...
  function renderAttention(status) {
    ...
    if (totals.conflicts) {
      rows.push(h('div', { class: 'attention-row' },
        h('div', {},
          ...
          h('div', { class: 'muted' }, 'DigiMem keeps syncing everything else.')),
    ...
    if (notify.supported() && notify.permission() === 'default') {
      rows.push(h('div', { class: 'attention-row' },
        h('div', { class: 'muted' },
          'DigiMem can tell you when this happens, even when this window is behind something else.'),
