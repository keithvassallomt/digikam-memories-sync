"""The long-running Face Sync process.
...
def service_is_live(info: dict[str, Any] | None, timeout: float = 1.5) -> bool:
    """Confirm that something is actually answering as Face Sync."""
    if not info:
        return False
    request = urllib.request.Request(
        service_url(info) + "api/health",
        headers={"X-Face-Sync-Token": str(info.get("token", ""))},
    )
    try:
...
def run_service(
    ...
    *,
    ...
    if not lock.acquire():
        ...
        if existing:
            print(f"Face Sync is already running at {service_url(existing)}", file=sys.stderr)
        else:
            print("Face Sync is already running.", file=sys.stderr)
