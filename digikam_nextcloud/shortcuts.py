"""Application-menu shortcuts that open the Face Sync interface.
...
def _command(config_dir: str | Path | None = None) -> list[str]:
    command = [sys.executable, "-m", "digikam_nextcloud", "ui"]
    if config_dir:
...
\t<string>com.keithvassallo.face-sync</string>
...
def _install_windows(config_dir: str | Path | None, path: Path) -> None:
    ...
    parts = [executable, "-m", "digikam_nextcloud", "ui"]
    if config_dir:
