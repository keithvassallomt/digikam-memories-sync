"""Start Face Sync when the user logs in.
...
SERVICE_NAME = "face-sync"
LAUNCH_AGENT_ID = "com.keithvassallo.face-sync"
WINDOWS_RUN_KEY = r"Software\Microsoft\Windows\CurrentVersion\Run"
WINDOWS_VALUE = "FaceSync"
...
def _systemd_unit(config_dir: str | Path | None) -> str:
    ...
    return f"""[Unit]
Description=Face Sync between digiKam and Nextcloud Memories
...
def _windows_command(config_dir: str | Path | None) -> str:
    ...
    parts = [executable, "-m", "digikam_nextcloud", "service"]
    if config_dir:
