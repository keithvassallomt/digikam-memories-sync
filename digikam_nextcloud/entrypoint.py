...
def _ui(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="face-sync ui", description="Open the interface")
...
def _service(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="face-sync service", description="Run the Face Sync background service"
    )
    parser.add_argument("--config-dir", type=Path)
    parser.add_argument("--port", type=int)
    parser.add_argument("-v", "--verbose", action="store_true")
...
def _autostart(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="face-sync autostart", description="Start Face Sync when you log in"
    )
...
def _shortcuts(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="face-sync shortcuts", description="Application-menu shortcut"
    )
