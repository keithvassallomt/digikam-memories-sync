...
export function create({ go, refresh }) {
  ...
  element.append(
    ...
    h('p', { class: 'lead' }, 'Face Sync keeps syncing everything else. These faces need you.'),
    body,
  );
    ...
    try {
      ...
      if (!inbox.total) {
        replace(body, card(
          ...
          h('p', { class: 'muted' }, 'Every face Face Sync found agrees in both libraries.')));
        return;
      }
      ...
      if (notify.supported() && notify.permission() === 'default') {
        parts.push(card(h('div', { class: 'row' },
          h('div', {},
            ...
            h('div', { class: 'muted' },
              'Face Sync can notify you while this window is behind another one.')),
