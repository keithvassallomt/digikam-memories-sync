...
export function create({ go, refresh }) {
  ...
  element.append(h('h1', {}, 'Settings'),
    h('p', { class: 'lead' }, 'Set these once. Face Sync checks them before every sync.'),
    message, body);
  ...
  function automationCard(settings) {
    ...
    return card(
      ...
      settingRow('Check for changes every',
        'How often Face Sync asks Nextcloud whether anything moved.',
  ...
  function notificationsCard(settings) {
    ...
    const description = {
      granted: 'On. Used while Face Sync is open but behind another window.',
      denied: 'Refused by the browser. Face Sync will use desktop notifications instead.',
      default: 'Not asked yet. Used while Face Sync is open but behind another window.',
  ...
  function serviceCard(service) {
    ...
    return card(
      ...
      settingRow('Start Face Sync when I log in',
